...
def eval_threading(Tup, SE_ordered, start_residues, file_out):
    # Evaluate threading
    zero_coordinates_SEM(SE_movers[('seq_chain_Nic96',1)]['main'])  

    print(len(SE_ordered))
    #all_polarities = [i for i in itertools.product([-1, 1],repeat=len(SE_ordered)) if (i[0]==1 and i[1]==1 and i[2]==1 and i[5]==-1)]
    all_polarities = [[1,1,1,1,1,1,1]]
    print('polarities', all_polarities, len(all_polarities))   
...
pdb_IR = f'{data_dir}/yeast-spoke-thread2-mdff-step13-noH-fix-MS1000-d8-align.pdb'
...
# Three levels
# 1. Pick chains for SE
# 2. Pick chain for threading
# 3. Pick residues


chain_perms_sel = [['R','R','R','R','R','R','R']]
...
SE_movers = {}
SE_movers[('seq_chain_Nic96',1)] = {'main':[]}
...
xl = StructuralElementCrossLinksRestraint(root_hier, f'{data_dir}/XLs_all_2020_sel_Nic96_Nup53_Nup59_Nup100_Nup116_Nup145N.csv', chain_mapping)
xl.add_to_model()
...
start_res = [[16, 29, 35, 74, 80, 120,155],[16, 29, 35, 76, 80, 120,155],]
...
print(all_options[opt], opt, scores_all)
zero_coordinates_SEM(SE_movers[('seq_chain_Nic96',1)]['main'])
...
for assign, perms in sel_assign.items():
    ...
    for ii, iters in enumerate(itertools.product(*perms.values())):
      if ii == sel_2:
        ...
        out_assign = open(f'enumeration_perm_{chains_label}_copy{copy_ch}_{perm_label}_resi{resi_5}.csv','w')
        ...
        limits_new[6][0]=resi_5+38
        limits_new[6][1]=resi_5+44
        print('limits_new', limits_new)
        ...
        print('first_row', start_res[0,:])
        print('last_row', start_res[-1,:])
        print('shape', np.shape(start_res))
